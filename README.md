...
**The public bundle currently needs no variables at all.** `VITE_FORM_ENDPOINT` and
`VITE_FORM_ACCESS_KEY` were removed in Phase 1c: the enquiry form now posts to this site's own API
at `/api/enquiries`, so there is no third-party form provider in the path.

> **Every `VITE_`-prefixed variable is inlined into the client bundle and is public.**
> Never put a private API key, an SMTP credential or anything you would not print on the page here.

**If the API is unreachable the form still works safely**: it reports a clear error and offers a
...
`ADMIN_SEED_*`, `CORS_ORIGIN`, `SMTP_*`, `NOTIFY_EMAIL`). Those are server-side secrets and must
**never** be given a `VITE_` prefix - see [`docs/admin.md`](docs/admin.md#environment-variables).

---

## Admin CMS

Phases 1a–1f shipped a database-backed admin panel. **Full manual: [`docs/admin.md`](docs/admin.md).**
...
- [ ] Deploy `server/` somewhere the site can reach at `/api` - the catalogue, the contact details and
      the enquiry form all depend on it. A static-only deploy now shows a catalogue error state
- [ ] Set `SMTP_*` and `NOTIFY_EMAIL` in `server/.env`, or new enquiries are stored with nobody emailed
- [ ] Save a real WhatsApp number at `#admin/settings` - the floating WhatsApp button stays hidden
      until one is set, because the seeded value is a placeholder
...
- [ ] Configure SPF, DKIM and DMARC on the sending domain, or the enquiry notifications land in spam
...
| `feat/admin-cms-phase1` | **Current work.** Admin CMS Phases 1a–1c - `server/` API, admin routes, public catalogue wired to the API, image upload, site settings, enquiry capture. Not merged |
